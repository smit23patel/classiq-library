...
def _get_qmod_path_for_metadata(metadata_path: Path) -> Path:
    ...
    if not qmod_path.exists():
        raise RuntimeError(
            f"Could not find qmod file for metadata file {metadata_path}. Expected at {qmod_path}"
        )
    return qmod_path
...
def _is_json(data: str) -> bool:
    ...
    try:
        ...
        raise RuntimeError("Invalid JSON data provided")
    return True
    ...
    for metadata_path in directory.rglob("*.metadata.json"):
        ...
        if single_metadata.get("qmod_dialect") is None:
            try:
                ...
                raise RuntimeError(
                    f"Failed to determine qmod type for {qmod_path}: {exc}"
                ) from exc
